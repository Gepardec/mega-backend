# Regular Working Times Aggregate

## Purpose

TBD — defines the `RegularWorkingTimes` aggregate that wraps a list of `RegularWorkingTime` entries and exposes temporal query methods for determining which working time entry is effective on a given date or payroll month.
