# Month-End Absent Employee Auto-Completion

## Purpose

Defines the use case and scheduler that automatically complete open month-end tasks for employees who were absent for an entire calendar month, and specifies the outbound port through which the `monthend` BC retrieves qualifying absence data from the `worktime` BC.
