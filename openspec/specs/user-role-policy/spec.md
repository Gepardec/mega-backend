...
Defines the user role policy capability for assigning and preserving roles during sync, including office-management email matching and project-lead role assignment coordination across the `user` and `project` subdomains.
...
The system SHALL keep project-lead role policy in `UserRolePolicyService` in `user.domain.services`. Given a `User` and whether that user should currently be a lead, it SHALL return an updated `User` only when a change is required.

#### Scenario: User who became a lead has PROJECT_LEAD granted
- **WHEN** `UserRolePolicyService.updateProjectLeadRoleIfNeeded(user, true)` is called and the user does not hold `Role.PROJECT_LEAD`
- **THEN** the returned value contains a user that holds `Role.PROJECT_LEAD`

#### Scenario: User who is no longer a lead has PROJECT_LEAD revoked
- **WHEN** `UserRolePolicyService.updateProjectLeadRoleIfNeeded(user, false)` is called and the user holds `Role.PROJECT_LEAD`
- **THEN** the returned value contains a user that no longer holds `Role.PROJECT_LEAD`

#### Scenario: User whose lead status already matches remains unchanged
- **WHEN** `UserRolePolicyService.updateProjectLeadRoleIfNeeded(user, shouldBeLead)` is called and the user's `PROJECT_LEAD` role already matches `shouldBeLead`
