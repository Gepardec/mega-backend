...
## Purpose
Defines how the active payroll month is resolved for authenticated actors. The payroll month drives which calendar month is presented as the current billing period in the monthend workflow. Resolution rules differ by role: employees use open task state, project leads always see the previous month.
