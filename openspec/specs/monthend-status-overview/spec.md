...
- **Employee overview**: returns all `MonthEndTask` records where the employee is the `subjectEmployeeId`. This covers their own `EMPLOYEE_TIME_CHECK` tasks (where they are also the eligible actor) and `LEISTUNGSNACHWEIS` and `PROJECT_LEAD_REVIEW` tasks about them (where they are the subject but not eligible).
...
#### Scenario: Employee overview includes LEISTUNGSNACHWEIS tasks about that employee
- **WHEN** an employee requests their month-end status overview for a month that contains `LEISTUNGSNACHWEIS` tasks where that employee is the subject
- **THEN** the system returns those `LEISTUNGSNACHWEIS` tasks in the employee's overview
...
#### Scenario: Lead sees EMPLOYEE_TIME_CHECK tasks as read-only
- **WHEN** the lead overview contains an `EMPLOYEE_TIME_CHECK` task
- **THEN** that overview entry has `canComplete` set to `false`

#### Scenario: Lead sees LEISTUNGSNACHWEIS tasks as actionable
- **WHEN** the lead overview contains a `LEISTUNGSNACHWEIS` task for a project they lead
...
- **THEN** the `EMPLOYEE_TIME_CHECK` task has `canComplete` set to `true` (they are the eligible employee)
- **THEN** the `LEISTUNGSNACHWEIS` task has `canComplete` set to `true` (they are in the eligible lead set)
...
#### Scenario: Subject-only actor sees canComplete false for PROJECT_LEAD_REVIEW and LEISTUNGSNACHWEIS tasks
- **WHEN** an employee requests their status overview and an entry corresponds to a `PROJECT_LEAD_REVIEW` or `LEISTUNGSNACHWEIS` task where they are the subject employee but not in `eligibleActorIds`
- **THEN** that overview entry has `canComplete` set to `false`
