...
The system SHALL include the month-end task identity, task type, task status, project reference, and subject employee reference when present in each overview entry.

#### Scenario: Review task entry identifies the employee and project
...
#### Scenario: Abrechnung entry identifies the project without a subject employee
- **WHEN** the overview contains an `ABRECHNUNG` task
- **THEN** that overview entry includes the associated project reference and no subject employee reference
