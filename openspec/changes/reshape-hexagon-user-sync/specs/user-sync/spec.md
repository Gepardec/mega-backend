...
- **THEN** regular working times are left to on-demand provider lookup
...
**Reason**: User activity is now derived from persisted employment periods, and employees with email must remain available locally for historical date-based use cases.
**Migration**: Replace disable/status transitions with employment-period synchronization and date-based activity checks.

### Requirement: Use case is decoupled from Quarkus infrastructure
**Reason**: User sync should now behave like the other hexagon application services by owning CDI wiring, transactions, and logging at the use-case boundary.
**Migration**: Inject the sync use case as a CDI-managed application service and keep infrastructure concerns out of the aggregate model rather than out of the application-service layer.
