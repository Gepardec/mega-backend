...
- **THEN** the use case returns a `WorkTimeReport` containing one `WorkTimeEntry` per (employee × project) combination where the employee booked time on one of the lead's projects that month
...
### Requirement: WorkTimeReport is a flat (employee × project) model
The `WorkTimeReport` domain model SHALL hold a `YearMonth` and a list of `WorkTimeEntry` records. Each `WorkTimeEntry` SHALL contain an `EmployeeRef` (UserId, display name), a `ProjectRef` (ProjectId, display name), `billableHours` (double), `nonBillableHours` (double), and `employeeMonthTotalHours` (double). The model SHALL NOT include pre-computed totals or percentages.

#### Scenario: WorkTimeEntry holds the correct fields
- **WHEN** a `WorkTimeEntry` is constructed
- **THEN** it contains `EmployeeRef`, `ProjectRef`, `billableHours`, `nonBillableHours`, and `employeeMonthTotalHours`
