...
class EmployeeResourceTest {
    ...
    class UploadInternalRate_POST {
        ...
        private static final String VALID_HOURLY_RATES_CSV_WITH_SEMICOLONS = """
                #ZEPMitarbeiterId;neuerStundensatz;gueltigAb YYYY-MM-DD
                005-wbruckmueller;72.00;2025-01-01
                102-funger;20.00;2025-12-31
                """;

        private static final String VALID_HOURLY_RATES_CSV_WITH_BLANK_LINES = """
                #ZEPMitarbeiterId,neuerStundensatz,gueltigAb YYYY-MM-DD
        ...
        private static final String CSV_WITH_MULTIPLE_ERRORS_AND_BLANKS = """
                #ZEPMitarbeiterId,neuerStundensatz,gueltigAb YYYY-MM-DD
                ,,
                ...
                102-funger,,2025-12-31
                """;
        private static final String CSV_WITH_BLANK_LINES_AND_ERROR = """
                #ZEPMitarbeiterId,neuerStundensatz,gueltigAb YYYY-MM-DD
        ...
        private static final String CSV_WITH_INVALID_NUMBER = """
                #ZEPMitarbeiterId,neuerStundensatz,gueltigAb YYYY-MM-DD
                005-wbruckmueller,not-a-number,2025-01-01
                """;

        private static final String CSV_WITH_INVALID_DATE = """
                #ZEPMitarbeiterId,neuerStundensatz,gueltigAb YYYY-MM-DD
                005-wbruckmueller,72.00,not-a-date
                """;

        private static final String CSV_WITH_EMPTY_FIELDS = """
                #ZEPMitarbeiterId,neuerStundensatz,gueltigAb YYYY-MM-DD
                005-wbruckmueller,,2025-01-01
                ,72.00,2025-01-01
                005-wbruckmueller,72.00,
                """;

        private static final String CSV_WITH_EXTRA_COLUMNS = """
                #ZEPMitarbeiterId,neuerStundensatz,gueltigAb YYYY-MM-DD
                005-wbruckmueller,72.00,2025-01-01,extra-column
                """;

        private static final String CSV_WITH_ONLY_HEADER = """
                #ZEPMitarbeiterId,neuerStundensatz,gueltigAb YYYY-MM-DD
                """;
        ...
        @Test
        void uploadCorrectInternalRateWithCommas() {
            ...
            when(userService.findByZepIds(any()))
            ...
            verify(zepService).updateEmployeeHourlyRate("005-wbruckmueller", 72.00D, "2025-01-01");
            verify(zepService).updateEmployeeHourlyRate("102-funger", 20.00D, "2025-12-31");
        }

        @Test
        void uploadCorrectInternalRateWithSemicolons() {
            final User user = createUserForRole(Role.OFFICE_MANAGEMENT);
            when(userContext.getUser()).thenReturn(user);
            final EmployeeDto userAsEmployee = createEmployeeForUser(user);
            when(employeeService.getEmployee(anyString())).thenReturn(mapper.mapToDomain(userAsEmployee));
            ...
            given()
                    .multiPart(buildMultipartSpec(VALID_HOURLY_RATES_CSV_WITH_SEMICOLONS))
                    .when()
                    .post("/employees/bulkUpdate")
                    .then()
                    .assertThat()
        ...
        @Test
        void uploadCorrectInternalRateWithBlankLines() {
            final User user = createUserForRole(Role.OFFICE_MANAGEMENT);
            when(userContext.getUser()).thenReturn(user);
            final EmployeeDto userAsEmployee = createEmployeeForUser(user);
            when(employeeService.getEmployee(anyString())).thenReturn(mapper.mapToDomain(userAsEmployee));
            ...
            given()
                    .multiPart(buildMultipartSpec(VALID_HOURLY_RATES_CSV_WITH_BLANK_LINES))
                    .when()
                    .post("/employees/bulkUpdate")
                    .then()
                    .assertThat()
        ...
        @Test
        void uploadInternalRate_withBlankLinesAndError_reportsCorrectOriginalLineNumber() {
            final User user = createUserForRole(Role.OFFICE_MANAGEMENT);
            when(userContext.getUser()).thenReturn(user);
            final EmployeeDto userAsEmployee = createEmployeeForUser(user);
            when(employeeService.getEmployee(anyString())).thenReturn(mapper.mapToDomain(userAsEmployee));
            ...
            given()
                    .multiPart(buildMultipartSpec(CSV_WITH_BLANK_LINES_AND_ERROR))
                    .when()
                    .post("/employees/bulkUpdate")
                    .then()
                    .assertThat()
                    ...
                    .body("message", anyOf(
                            equalTo("Error: The uploaded file is not formatted correctly"),
                            equalTo("Fehler: Die hochgeladene Datei ist nicht richtig formatiert")))
                    // The error is on line 6 in the original file
                    .body("location", equalTo(List.of(6)));
        }

        @Test
        void uploadInternalRate_withMultipleErrorsAndBlankLines_reportsAllCorrectLineNumbers() {
            final User user = createUserForRole(Role.OFFICE_MANAGEMENT);
            when(userContext.getUser()).thenReturn(user);
            final EmployeeDto userAsEmployee = createEmployeeForUser(user);
            when(employeeService.getEmployee(anyString())).thenReturn(mapper.mapToDomain(userAsEmployee));
            ...
            given()
                    .multiPart(buildMultipartSpec(CSV_WITH_MULTIPLE_ERRORS_AND_BLANKS))
                    .when()
                    .post("/employees/bulkUpdate")
                    .then()
                    .assertThat()
                    ...
                    .body("message", anyOf(
                            equalTo("Error: The uploaded file is not formatted correctly"),
                            equalTo("Fehler: Die hochgeladene Datei ist nicht richtig formatiert")))
                    .body("location", equalTo(List.of(2, 6, 9)));
        }

        @Test
        void uploadInternalRate_withInvalidNumberFormat_returnsValidationError() {
            final User user = createUserForRole(Role.OFFICE_MANAGEMENT);
            when(userContext.getUser()).thenReturn(user);
            final EmployeeDto userAsEmployee = createEmployeeForUser(user);
            when(employeeService.getEmployee(anyString())).thenReturn(mapper.mapToDomain(userAsEmployee));
            ...
            given()
                    .multiPart(buildMultipartSpec(CSV_WITH_INVALID_NUMBER))
                    .when()
                    .post("/employees/bulkUpdate")
                    .then()
                    .assertThat()
        ...
        @Test
        void uploadInternalRate_withInvalidDateFormat_returnsValidationError() {
            final User user = createUserForRole(Role.OFFICE_MANAGEMENT);
            when(userContext.getUser()).thenReturn(user);
            final EmployeeDto userAsEmployee = createEmployeeForUser(user);
            when(employeeService.getEmployee(anyString())).thenReturn(mapper.mapToDomain(userAsEmployee));
            ...
            given()
                    .multiPart(buildMultipartSpec(CSV_WITH_INVALID_DATE))
                    .when()
                    .post("/employees/bulkUpdate")
                    .then()
                    .assertThat()
        ...
        @Test
        void uploadInternalRate_withEmptyFields_returnsValidationError() {
            final User user = createUserForRole(Role.OFFICE_MANAGEMENT);
            when(userContext.getUser()).thenReturn(user);
            final EmployeeDto userAsEmployee = createEmployeeForUser(user);
            when(employeeService.getEmployee(anyString())).thenReturn(mapper.mapToDomain(userAsEmployee));
            ...
            given()
                    .multiPart(buildMultipartSpec(CSV_WITH_EMPTY_FIELDS))
                    .when()
                    .post("/employees/bulkUpdate")
                    .then()
                    .assertThat()
                    ...
                    .body("message", anyOf(
                            equalTo("Error: The uploaded file is not formatted correctly"),
                            equalTo("Fehler: Die hochgeladene Datei ist nicht richtig formatiert")))
                    .body("location", equalTo(List.of(2, 3, 4)));
        }

        @Test
        void uploadInternalRate_withTooManyColumns_returnsValidationError() {
            final User user = createUserForRole(Role.OFFICE_MANAGEMENT);
            when(userContext.getUser()).thenReturn(user);
            final EmployeeDto userAsEmployee = createEmployeeForUser(user);
            when(employeeService.getEmployee(anyString())).thenReturn(mapper.mapToDomain(userAsEmployee));
            ...
            given()
                    .multiPart(buildMultipartSpec(CSV_WITH_EXTRA_COLUMNS))
                    .when()
                    .post("/employees/bulkUpdate")
                    .then()
                    .assertThat()
        ...
        @Test
        void uploadInternalRate_withOnlyHeader_returnsEmptyFileError() {
            final User user = createUserForRole(Role.OFFICE_MANAGEMENT);
            when(userContext.getUser()).thenReturn(user);
            final EmployeeDto userAsEmployee = createEmployeeForUser(user);
            when(employeeService.getEmployee(anyString())).thenReturn(mapper.mapToDomain(userAsEmployee));
            ...
            given()
                    .multiPart(buildMultipartSpec(CSV_WITH_ONLY_HEADER))
                    .when()
                    .post("/employees/bulkUpdate")
                    .then()
                    .assertThat()
                    .statusCode(400)
                    .body("message", anyOf(
                            equalTo("Error: The file is empty"),
                            equalTo("Fehler: Die Datei ist leer")));
        }
